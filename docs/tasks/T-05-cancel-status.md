...
## Đã làm gì
- `lib/status.ts`: `canTransition(from, to)` (bảng `booked→in_service→done`,
  `cancelled`/`no_show` chỉ từ `booked`) + `canCustomerCancel(startAt, now)`
  (`now <= startAt - CANCEL_CUTOFF_MIN*60`, hằng số `CANCEL_CUTOFF_MIN=120`
  khai báo tại đây). Hàm thuần, không import D1.
- `routes/cancel.ts`: `POST /api/bookings/:id/cancel` — load item theo id
  (404 nếu không có), `canTransition` chặn huỷ cái không ở `booked` (409
  `INVALID_TRANSITION`), `canCustomerCancel` với `now = Date.now()` phía
  worker chặn cutoff (409 `CANCEL_TOO_LATE`), thành công thì
  `UPDATE ... SET status='cancelled', cancelled_at=?`.
- `routes/admin-status.ts`: gộp `POST /api/admin/bookings/:id/status` (body
  `{status}` ∈ `in_service|done|no_show`, validate qua `canTransition`, sai
  giá trị → 422 `VALIDATION`, sai transition → 409 `INVALID_TRANSITION`) và
  `POST /api/admin/bookings/:id/cancel` (huỷ miễn cutoff, vẫn qua
  `canTransition` nên huỷ cái đã huỷ/đã done vẫn 409).
- Đăng ký 2 dòng vào `registerRoutes()` trong `routes/index.ts` (`cancel`,
  `adminStatus`) — không sửa dòng nào khác; một agent song song (T-08) cũng
  thêm dòng của họ vào cùng file, không xung đột.
- `tests/api/cancel-status.test.ts`: 16 test, phủ đủ danh sách trong card
  (huỷ thành công + slot mở lại ngay qua `computeAvailability`, cutoff 409,
  ranh giới 120' chẵn vẫn hợp lệ, admin miễn cutoff, huỷ cái đã huỷ/đã done,
  404 not-found, 4 transition admin hợp lệ/không hợp lệ, no_show terminal
  không có logic mở lại slot, 422 cho status rác).

### Kết quả
- `npm run typecheck`: xanh.
- `npm test -- tests/api/cancel-status.test.ts`: 16/16 xanh.
- `npm test` (toàn bộ): các file tôi động vào + toàn bộ suite khác của T-01→T-06
  đều xanh (117/117 khi chạy riêng, không tính file walkin.test.ts của T-08 —
  file đó thuộc task khác đang chạy song song, ngoài `touches` của T-05, không
  đụng tới).
- Mutation-test (3 lần, mỗi lần phá xong khôi phục nguyên trạng):
  1. Tắt kiểm tra cutoff ở server (`routes/cancel.ts`) → 1 test đỏ đúng
     (`huỷ trong vòng 2 tiếng trả 409 CANCEL_TOO_LATE`).
  2. Tắt `canTransition` trong `routes/cancel.ts` (cho phép huỷ cái đã
     huỷ/đã done) → 2 test đỏ đúng (`huỷ một booking đã huỷ...`, `huỷ một
     booking đã done...`).
  3. Trong `lib/status.ts`, cho `from === 'in_service'` luôn `return true`
     (mô phỏng done↔in_service tuỳ tiện) → 1 test đỏ đúng (`admin đánh dấu
     no_show một booking đang in_service trả 409 INVALID_TRANSITION`).
  Cả 3 đột biến đều bị bộ test bắt được (không có lỗ hổng lọt qua). Sau mỗi
  lần đã khôi phục code về đúng bản gốc và xác nhận lại toàn bộ suite xanh.
